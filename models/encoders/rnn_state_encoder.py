...
def _build_pack_info_from_dones(
    ...
    last_episode_in_batch_mask = (
        torch.div(episode_starts + (lengths - 1) * N, N, rounding_mode="trunc")
    ) == (T - 1)
...
def build_rnn_inputs(
    ...
    rnn_states = torch.where(
        not_dones.view(1, -1, 1).index_select(1, episode_starts).bool(),
        rnn_states,
...
class RNNStateEncoder(nn.Module):
    ...
    def single_forward(
        ...
        hidden_states = torch.where(
            masks.bool().view(1, -1, 1), hidden_states, hidden_states.new_zeros(())
        )
...
def build_rnn_state_encoder(
    ...
    if rnn_type == "gru":
        return GRUStateEncoder(input_size, hidden_size, num_layers, dropout)
    elif rnn_type == "lstm":
        return LSTMStateEncoder(input_size, hidden_size, num_layers, dropout)
    else:
